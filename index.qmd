---
title: "DATA PROGRAMMING: <br>GROUP TASK"
...
wordcloud(words = party_coeff_df$party_abbrev,  # Texts will represent parties
          freq = party_coeff_df$size,           # Basing frequencies on abs values of coeffs
          min.freq = 0, 
          max.words = 12,  # I will restrict anyways. Not sure if it's necessary
          random.order = FALSE,
          rot.per = 0.5,
          scale = c(2.8, 0.60),
          colors = party_coeff_df$color)


title(main = "Party Coefficients for Vote Share ~ Turnout", 
      sub = "**Parties are sized on the basis of the absolute values of their respective coefficients**", 
...
legend(x = 0.95, y = 0.65,  # positioning the legend
       legend = names(color_pal),  # to translate legend keys from color_pal
       fill = color_pal,           # filling the legend keys with color_pal
       cex = 0.8,                  # legend text size
       text.font = 2, 
       text.col = "#636363",
       bty = "n",                  #  box around the legend
       ncol = 1) 
...
ggplot(poll_calibration, aes(x = error)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black", alpha = 0.7) +
  geom_density(color = "darkblue", size = 1) +  
  theme_minimal() +
  labs(
    title = "Survey Error Distribution ",
    x = "Error (Estimate - Actual Vote) (%)",
    y ="Density")
```
...
ggplot(data = polling_error_by_party, aes(x = reorder(party, mean_abs_error), y = mean_abs_error, fill = party)) +
  geom_col(color = "black") + 
  geom_errorbar(aes(ymin = mean_abs_error - sd_abs_error, ymax = mean_abs_error + sd_abs_error), 
                width = 0.2, color = "darkblue") +
  theme_minimal() +  
  labs(
    title = "Mean Absolute Error by Political Party",
    x = "Political Party",
    y = "Mean Absolute Error",
    fill = "Party"
  ) +
...
> How the multiparty system changed throughout the elections according to the polarization index?
...
![](https://c.tenor.com/ShzdJcrguswAAAAd/tenor.gif)
