...
election_data_tidy <- election_data_tidy |> 
  ...
  unite(col = "cod_mun", codigo_ccaa, codigo_provincia, codigo_municipio, sep = "-", remove = FALSE)
...
election_data_tidy_woNA <- election_data_tidy |> 
  drop_na(votes) |> 
  mutate(siglas = if_else(siglas == "C's", "CS", siglas))
...
# Do it more efficiently if all of us are using the wo_na
```
