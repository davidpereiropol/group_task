...
catalunya <- election_data_tidy |> 
  ...
  mutate(
    ...
    # Asignar nombres a las provincias
    province = case_when(
  ...
  # Filtrar datos para Cataluña y evitar duplicados por partido
  filter(codigo_ccaa == "09") |> 
  distinct(date, cod_mun, party_recoded, .keep_all = TRUE)

# Calcular los votos totales y porcentaje de votos por provincia y fecha
...
g <- ggplot(catalunya_votes, aes(x = date, y = vote_percentage, 
  ...
  geom_point(size = 3) +
  facet_wrap(~ province, ncol = 2) + # Facetas por provincia
  scale_color_manual(
    values = c(
      "PP" = "#1db4e8",
      "PSOE" = "#c30505",
