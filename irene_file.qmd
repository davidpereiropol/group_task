...
```{r}
# Primero calculamos el porcentaje de municipios ganados
second_combined2 <- second_combined %>%
  group_by(first, date) %>%
  mutate(
    total_municipalities_per_election = sum(num_municipalities),
    percent_municipalities = (num_municipalities / total_municipalities_per_election) * 100
  ) %>%
  ungroup()

second_combined2$date <- factor(second_combined2$date, levels = unique(second_combined2$date))

# Crear el gráfico de barras agrupadas con porcentaje
ggplot(second_combined2, aes(x = date, y = percent_municipalities, fill = second)) +
  ...
  facet_wrap(
    ~ first,
    scales = "free_y",
    labeller = labeller(first = c(PP = "PP Primero", PSOE = "PSOE Primero"))
  ) +
  geom_text(
    aes(label = paste0(round(percent_municipalities, 1), "%")), 
    position = position_stack(vjust = 0.5), 
